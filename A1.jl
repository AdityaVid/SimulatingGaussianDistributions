...
function var(D::Vector{Float64})
	...
	length = 0
	for i in D
		sum_square = sum_square + (i - mean_val)^2
		length = length + 1
	end
	variance = sum_square / (length - 1)
	return variance
...
# ╔═╡ 2f0b03d8-4862-4a22-a9ae-13a8a96e6760
function confinterval(D::Vector{Float64}, variance::Int, samples)
	Z_score = 1.96
	sample_avg = mean(D)
	std_dev = sqrt(variance)
	error = Z_score * std_dev / sqrt(samples)
	return [sample_avg - error, sample_avg + error, sample_avg]
end
...
let # 1d, 1e
	n = 30
	variance = 10
	gd = GaussianDistribution(0, sqrt(variance))
	dataset = sample(gd, n)
	for i in 1:n
		println(dataset[i])
	end
	confinterval(dataset, variance, n)
end

# ╔═╡ f80aed6f-5ed1-4981-ac52-21279a2eb7d5
let
	Chebyshevs_error = sqrt((10)/(0.05*30))
	[0.495515 - Chebyshevs_error, 0.495515 + Chebyshevs_error, Chebyshevs_error]
end
